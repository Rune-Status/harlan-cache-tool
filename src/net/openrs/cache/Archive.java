...
public class Archive {
	...
	public static Archive decode(ByteBuffer buffer, int size) {
		...
		int chunksSizes = chunks * size * 4;//1024
		buffer.position(buffer.limit() - 1 - chunksSizes);
		for (int chunk = 0; chunk < chunks; chunk++) {
			...
			for (int id = 0; id < size; id++) {
				...
				if (delta > size) {
					System.out.println("Invalid delta detected! returning null");
					return null;
				}
	...
	public ByteArrayOutputStream asOutputStream() {
		...
		for (ByteBuffer buf : this.entries) {
			try {
	...
	public ByteBuffer encode() throws IOException { // TODO: an implementation that can use more than one chunk
		...
		try {
			...
			for (ByteBuffer entrie : entries) {
				/* copy to temp buffer */
				byte[] temp = new byte[entrie.limit()];
				entrie.position(0);
				try {
					entrie.get(temp);
				} finally {
