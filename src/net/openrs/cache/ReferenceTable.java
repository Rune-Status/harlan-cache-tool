...
public class ReferenceTable {
	...
	public static class Entry {
		...
		public String toString() {
			return "[Entry] id: "+identifier+ " crc: "+crc+" children: "+childSize()+ " max children:  " + capacity();
		}
		...
		public  SortedMap<Integer, ChildEntry> getChildEntries() {
			return entries;
		}
	...
	public static ReferenceTable decode(ByteBuffer buffer) {
		...
		int[] identifers = new int[ids.length];
		int index = 0;
		/* read the identifiers if present */
		if ((table.flags & FLAG_IDENTIFIERS) != 0)
			for (int id : ids) {
				table.entries.get(id).identifier = buffer.getInt();
				identifers[index] = table.entries.get(id).identifier;
				index++;
			}
		table.archiveIdentifier = new ArchiveIdentifier(identifers);
