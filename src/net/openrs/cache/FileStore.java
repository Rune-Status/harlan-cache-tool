...
public final class FileStore implements Closeable {
	...
	public static FileStore open(File root) throws FileNotFoundException {
		...
		if (!metaChannel.isOpen()) {
			System.out.println("meta channel was not opened!");
		}
	...
	public FileChannel[] getIndexChannels() {
	 ...
	 * @throws IOException if an I/O error occurs.
	 */
	public boolean write(int type, int id, ByteBuffer data) throws IOException {
		data.mark();
		if (!write(type, id, data, true)) {
			data.reset();
			return write(type, id, data, false);
		}
		return true;
	}
	...
	private boolean write(int type, int id, ByteBuffer data, boolean overwrite) throws IOException {
		...
		if (!channel.isOpen()) {
			System.out.println("channel is not open!: "+type+ " channel["+channel.toString()+"]");
			return false;
		}
		int nextSector = 0;
		...
		if (overwrite) {
			...
			else if (ptr >= channel.size())
				return false;

			ByteBuffer buf = ByteBuffer.allocate(Index.SIZE);
			FileChannelUtils.readFully(channel, buf, ptr);
		...
		channel.write(index.encode(), ptr);
	...
	public boolean isInvalidIndex(int type, int id) throws IOException {
		if ((type < 0 || type >= indexChannels.length) && type != 255)
			return true;
		if (id == -1) {
			return true;
	...
	public ByteBuffer read(int type, int id) throws IOException {
		if ((type < 0 || type >= indexChannels.length) && type != 255)
			throw new FileNotFoundException();
		if (id == -1) {
			throw new ArrayIndexOutOfBoundsException("Bad Part Size -1! File ID: "+ id + " File Type: "+ type+ " Index Size: " + Index.SIZE);
		}

		FileChannel channel = type == 255 ? metaChannel : indexChannels[type];

		if (!channel.isOpen()) {
			System.out.println("channel: "+type+" is not open!");
		}

		long ptr = id * Index.SIZE;
		if (ptr < 0 || ptr >= channel.size()) {
			JOptionPane.showMessageDialog(CacheViewer.getCacheViewer(), "Invalid file pointer!\n main_file_cache.idx" + type + "  is corrupted.");
			throw new ArrayIndexOutOfBoundsException("Bad Part Size! Part: " + ptr + " File ID: "+ id + " File Type: "+ type+ " Index Size: " + Index.SIZE + " Channel Size: " + channel.size());
		}
	...
	public void close() throws IOException {
		...
		metaChannel.close();
		System.out.println("channels were closed!");
	}
